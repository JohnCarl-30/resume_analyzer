...
Fill in `apps/api/.env` if you need to override defaults or configure optional features (database, OpenAI, R2, Clerk). No variables are strictly required — `PORT` defaults to `4000` and `APP_ORIGIN` defaults to `http://localhost:3000`, so the API will start without an `.env` file.

> **Do not trust the README for env var names.** The README mentions `VERTEX_AI_MODEL` but the actual code reads `AI_EXTRACTION_MODEL`. The real source of truth is `apps/api/src/config/env.ts` — check there for the exact schema, defaults, and required fields.
...
- **Clerk auth**: When `CLERK_SECRET_KEY` is set, `@clerk/backend` is used for token verification on protected routes.
- **Tests**: API has `"test": "vitest run"` in its package.json — run via `corepack pnpm --filter @resume-analyzer/api test`. Manual sanity/regression scripts also live in `src/tests/` and are run directly with `corepack pnpm exec tsx` (e.g., `corepack pnpm exec tsx src/tests/analyzer-sanity.ts`).
...
- **Tests**: Vitest with jsdom and `@testing-library/react`. Property-based tests use `fast-check`. Setup file is `vitest.setup.ts`. There is no `test` script in web's `package.json`; run tests with `npx vitest` from inside `apps/web/`.

## CI

- **`ci.yml`**: Runs on push to `main` and all PRs. Steps: `pnpm run typecheck` → `pnpm run build:web` → `pnpm run build:api`. Does NOT run tests.
- **`deploy.yml`**: On push to `main`, builds API Docker image and pushes to GHCR (`ghcr.io`).
...
- Do not run `corepack pnpm test` — no root test script exists; use `corepack pnpm --filter @resume-analyzer/api test` or run vitest inside `apps/web/`.
