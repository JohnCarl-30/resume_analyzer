...
- `apps/api`: NestJS API for uploads, parsing, analysis, AI-backed enhancement, and persistence (TypeORM + Neon).
...
- feature modules (`analysis/`, `account/`, `job-applications/`, `enhancement/`, `product-events/`): controller + service + repository per domain, registered in `AppModule`.
