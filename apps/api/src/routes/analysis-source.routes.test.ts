...
async function uploadAnalysis(app: Hono<AppEnv>, token?: string) {
  ...
  return app.request("/api/analysis/upload", {
    method: "POST",
    headers,
    body: form,
  });
...
describe("POST /api/analysis/upload", () => {
  ...
  it("requires sign-in before accepting an upload", async () => {
    const response = await uploadAnalysis(app);
  ...
  it("persists an uploaded analysis and redeems the account quota", async () => {
    const uploadResponse = await uploadAnalysis(app, "owner-token");
    ...
    const quotaResponse = await app.request("/api/account/analysis-quota", {
      headers: {
  ...
  it("rejects a second upload once the one-analysis quota is redeemed", async () => {
    const firstUpload = await uploadAnalysis(app, "owner-token");
    expect(firstUpload.status).toBe(201);

    const secondUpload = await uploadAnalysis(app, "owner-token");
...
describe("GET /api/analysis/:analysisId/source", () => {
  ...
  it("requires sign-in before returning a saved source file", async () => {
    ...
    const response = await app.request(`/api/analysis/${analysisId}/source`);
